...
function getDistinctClassList(elm) {
  ...
  return siblings.reduce((classList, childElm) => {
    ...
    return classList.filter((classItem) => !childElm.classList.contains(classItem));
 ...
 */
const getSelector = function createUniqueSelector(elm, options = {}) {
  if (!elm) {
    return '';
  }
  ...
  if (doc.nodeType === 11) { // DOCUMENT_FRAGMENT
    ...
    stack.push({ elm, doc });
    return stack.reverse().map((comp) => generateSelector(comp.elm, options, comp.doc));
  }
...
var matchesSelector = (function () {
  ...
  function getMethod(win) {
    let index; let candidate;
    const elProto = win.Element.prototype;
    const candidates = ['matches', 'matchesSelector', 'mozMatchesSelector', 'webkitMatchesSelector', 'msMatchesSelector'];
...
function uriParser(url) {
  // jshint maxstatements:19
  const original = url;
  let protocol = ''; let domain = ''; let port = ''; let path = ''; let query = ''; let
    hash = '';
  if (url.includes('#')) {
  ...
  return {
    original, protocol, domain, port, path, query, hash,
  };
}
...
var getFriendlyUriEnd = function getFriendlyUriEnd(uri = '', options = {}) {
  ...
  if (hash) {
    if (pathEnd && (pathEnd + hash).length <= maxLength) {
      return pathEnd + hash;
    } if (pathEnd.length < 2 && hash.length > 2 && hash.length <= maxLength) {
      return hash;
    }
    return;
  } if (domain && domain.length < maxLength && path.length <= 1) { // '' or '/'
    return domain + path;
  }
